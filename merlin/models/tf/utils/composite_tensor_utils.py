...
def type_spec_register(name, allow_overwrite=True):
    ...
    if allow_overwrite and name in type_spec._NAME_TO_TYPE_SPEC:
        type_spec._TYPE_SPEC_TO_NAME.pop(type_spec._NAME_TO_TYPE_SPEC.pop(name))
    return type_spec.register(name)


def auto_composite_tensor(cls=None, omit_kwargs=(), non_identifying_kwargs=(), module_name=None):
    ...
    if issubclass(cls, composite_tensor.CompositeTensor):
        ...
        class _AlreadyCTTypeSpec(AutoCompositeTensorTypeSpec):
            @property
            def value_type(self):
                return cls
        ...
        def _type_spec(obj):
            return _AlreadyCTTypeSpec.from_instance(obj, omit_kwargs, non_identifying_kwargs)

        cls._type_spec = property(_type_spec)  # pylint: disable=protected-access
        return cls

    clsid = (cls.__module__, cls.__name__, omit_kwargs, non_identifying_kwargs)
    ...
    class _GeneratedCTTypeSpec(AutoCompositeTensorTypeSpec):
        @property
        def value_type(self):
            return _registry[clsid]
    ...
    class _AutoCompositeTensor(cls, composite_tensor.CompositeTensor):
        ...
        @property
        def _type_spec(self):
            return _GeneratedCTTypeSpec.from_instance(self, omit_kwargs, non_identifying_kwargs)
