...
class PopularityBasedSampler(Block):
    def __init__(self, max_num_samples: int = 10, seed: Optional[int] = None, pre=None, post=None):
    ...
    def forward(self, positive, positive_id=None):
        ...
        _, _, negative_id = self.sampler.sample(self.labels)

        negative = self.to_call.embedding_lookup(torch.squeeze(negative_id))
        self.register_buffer("negative", negative)
        self.register_buffer("negative_id", negative_id)

        return negative, negative_id

    @property
    def sampler(self) -> LogUniformSampler:
        return LogUniformSampler(self.to_call.num_classes, self.max_num_samples)
