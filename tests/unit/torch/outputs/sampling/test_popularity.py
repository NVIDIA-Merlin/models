...
class TestLogUniformSampler:
    ...
    def test_sample_exceptions(self):
        ...
        with pytest.raises(ValueError):
            sampler.sample(torch.tensor([[10, 50, 150]]))
...
class TestPopularityBasedSampler:
    ...
    def test_forward_raises_runtime_error(self):
        ...
        with pytest.raises(RuntimeError):
            sampler.forward(torch.tensor([1.0]), torch.tensor([1]))
    ...
    def test_forward(self):
        ...
        negative, negative_id = sampler.forward(torch.tensor([1.0]), torch.tensor([1]))
