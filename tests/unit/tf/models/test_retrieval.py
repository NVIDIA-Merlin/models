...
@pytest.mark.parametrize("run_eagerly", [True, False])
def test_two_tower_with_avg_sequence_embeddings(run_eagerly, tmpdir):
    def _generate_data():
        import cudf
        import nvtabular as nvt
        ...
        df = cudf.DataFrame(
            {
                "channel_id": [1, 2, 3, 4, 9, 12, 4, 1, 1, 3, 4, 7, 11, 8] * 100,
                "channel_id_hist": [
                    [1, 3, 10],
                    [1, 5],
                    [4, 2, 1],
        ...
        joint_feats = ["channel_id_hist", "channel_id"] >> nvt.ops.Categorify(out_path=str(tmpdir))

        item_id = joint_feats["channel_id"] >> nvt.ops.TagAsItemID()
        user_feat = (
            joint_feats["channel_id_hist"]
            >> nvt.ops.TagAsUserFeatures()
            >> nvt.ops.AddMetadata([Tags.SEQUENCE])
        )

        weights = ["channel_id_hist_weights"] >> nvt.ops.AddMetadata([Tags.USER])

        outputs = item_id + user_feat + weights
        ...
        workflow.fit(train_dataset)
        return workflow.transform(train_dataset)

    train = _generate_data()
    ...
    embeddings = mm.Embeddings(
        ...
        sequence_combiner=mm.AverageEmbeddingsByWeightFeature.from_schema_convention(
            train.schema, "_weights"
        ),
    )

    emb_user = embeddings.select_by_tag(Tags.USER)
    user_inputs = mm.InputBlockV2(
        user_schema,
        categorical=emb_user,
    )
    query = mm.Encoder(user_inputs, mm.MLPBlock([tower_dim]))

    emb_item = embeddings.select_by_tag(Tags.ITEM)
    item_inputs = mm.InputBlockV2(item_schema.select_by_tag(Tags.CATEGORICAL), categorical=emb_item)
    candidate = mm.Encoder(item_inputs, mm.MLPBlock([tower_dim]))
    ...
    loader = mm.Loader(train, batch_size=16, shuffle=False, schema=train.schema)
